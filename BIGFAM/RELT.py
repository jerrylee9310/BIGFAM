...
def read_trait(trait_file):
    ...
    #     coltypes.append('f8')
    # trait = np.genfromtxt(trait_file, delimiter='\t', missing_values='NA')
    tt = pd.read_csv(trait_file, sep='\t')
    tt = tt.astype({"FID": str,
                    "IID": str,
                    "trait": float,})
    trait = tt.to_numpy()
...
v_y = np.sum(np.square(trait)) / float(n)
...
var_v_y = 2 * np.sum(np.square(sigma)) / float(n ** 2)
...
cov_v_g_v_y = 2 * cov_v_g_v_y / float(n)
